...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = f"Почитай статью '{post.title}' по адресу {post_url}\n\n " \
                      f"{cd['name']} комментирует: {cd['comments']}"
            send_mail(subject, message, 'romangvaramadze@gmail.com', [cd['to']])
            sent = True
...
def index(request):
    ...
    return render(request, 'blog/post/index.html', {'form':form})